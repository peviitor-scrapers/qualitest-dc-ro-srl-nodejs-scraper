...
const ANRAF_RECORD = {
  ...
  registrationNumber: 'J40/8760/2010',
  vatRegistered: true,
...
const CACHED_DATA = {
  cui: 39814543,
  name: 'QUALITEST DC RO S.R.L.',
  address: 'MUNICIPIUL BUCUREŞTI, SECTOR 2, GARA HERĂSTRĂU, NR.6, ET.1',
  registrationNumber: 'J40/8760/2010',
  caenCode: '6210',
  inactive: false,
  onrcStatusLabel: 'Funcțiune',
  administrators: [{ name: 'SHAI LIBERMAN', role: 'administrator' }],
  authorizedCaenCodes: ['6210', '6201', '6202', '6209', '7022']
};
...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 39814543, name: 'QUALITEST DC RO S.R.L.', statusLabel: 'Funcțiune' }
      ]));

      const results = await anaf.searchCompany('Qualitest');
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('Qualitest')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('Qualitest SRL');
      expect(capturedUrl).toContain(encodeURIComponent('Qualitest SRL'));
    });
  });
  ...
  describe('getCompanyFromANAF', () => {
    it('should return company data for valid CIF', async () => {
      ...
      const data = await anaf.getCompanyFromANAF('39814543');

      expect(data).toBeDefined();
      expect(data.cui).toBe(39814543);
      expect(data.name).toBe('QUALITEST DC RO S.R.L.');
    ...
    it('should throw after exhausting retries', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('39814543')).rejects.toThrow();
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('39814543');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('39814543', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('39814543')).rejects.toThrow();
    });
  });
});
